...
func convertManagerSpecToArgs(mSpec *ManagerSpec) map[string]string {
	...
	if mSpec.Controller != nil {
		...
		if mSpec.Controller.CacheSyncTimeout != nil {
			args["--cache-sync-timeout"] = mSpec.Controller.CacheSyncTimeout.String()
		}

		if mSpec.Controller.RecoverPanic != nil {
			args["--recover-panic"] = strconv.FormatBool(*mSpec.Controller.RecoverPanic)
		}
	...
	if mSpec.Health.LivenessEndpointName != "" {
		args["--liveness-endpoint-name"] = mSpec.Health.LivenessEndpointName
	}

	if mSpec.Health.ReadinessEndpointName != "" {
		args["--readiness-endpoint-name"] = mSpec.Health.ReadinessEndpointName
	}

	if mSpec.GracefulShutdownTimeout != nil {
		args["--graceful-shutdown-timeout"] = mSpec.GracefulShutdownTimeout.Duration.String()
	}

	if mSpec.LeaderElection != nil {
		if mSpec.LeaderElection.LeaderElect != nil {
		...
		if mSpec.LeaderElection.LeaseDuration.Duration != 0 {
			args["--leader-elect-lease-duration"] = mSpec.LeaderElection.LeaseDuration.Duration.String()
		}

		if mSpec.LeaderElection.RenewDeadline.Duration != 0 {
			args["--leader-elect-renew-deadline"] = mSpec.LeaderElection.RenewDeadline.Duration.String()
		}

		if mSpec.LeaderElection.RetryPeriod.Duration != 0 {
			args["--leader-elect-retry-period"] = mSpec.LeaderElection.RetryPeriod.Duration.String()
		}

		if mSpec.LeaderElection.ResourceLock != "" {
			args["--leader-elect-resource-lock"] = mSpec.LeaderElection.ResourceLock
		}
	}
	...
	if mSpec.Metrics.DiagnosticsAddress != "" {
		args["--diagnostics-address"] = mSpec.Metrics.DiagnosticsAddress
	}

	if mSpec.Metrics.InsecureDiagnostics {
		args["--insecure-diagnostics"] = strconv.FormatBool(mSpec.Metrics.InsecureDiagnostics)
	}
	...
	if mSpec.SyncPeriod != nil {
		args["--sync-period"] = mSpec.SyncPeriod.Duration.String()
	}
...
func convertArgsToManagerSpec(args map[string]string) *ManagerSpec {
	...
	for k, v := range args {
		...
		case strings.HasSuffix(k, "-concurrency"):
			...
			if val, err := strconv.Atoi(v); err == nil {
				if mSpec.Controller == nil {
					mSpec.Controller = &ControllerConfigurationSpec{}
				}

				if mSpec.Controller.GroupKindConcurrency == nil {
					mSpec.Controller.GroupKindConcurrency = make(map[string]int)
				}
		...
		case k == "--cache-sync-timeout":
			if duration, err := parseDurationString(v); err == nil {
				if mSpec.Controller == nil {
					mSpec.Controller = &ControllerConfigurationSpec{}
				}

				mSpec.Controller.CacheSyncTimeout = &duration
				processedArgs[k] = true
			}

		case k == "--recover-panic":
			if mSpec.Controller == nil {
				mSpec.Controller = &ControllerConfigurationSpec{}
			}

			if val, err := strconv.ParseBool(v); err == nil {
				mSpec.Controller.RecoverPanic = &val
				processedArgs[k] = true
			}
		...
		case k == "--liveness-endpoint-name":
			mSpec.Health.LivenessEndpointName = v
			processedArgs[k] = true

		case k == "--readiness-endpoint-name":
			mSpec.Health.ReadinessEndpointName = v
			processedArgs[k] = true

		case k == "--graceful-shutdown-timeout":
			if duration, err := parseDurationString(v); err == nil {
				mSpec.GracefulShutdownTimeout = &metav1.Duration{Duration: duration}
				processedArgs[k] = true
			}
		...
		case k == "--leader-elect":
			if val, err := strconv.ParseBool(v); err == nil {
				...
				mSpec.LeaderElection.LeaderElect = &val
				processedArgs[k] = true
			}
		...
		case k == "--leader-elect-resource-lock":
			if mSpec.LeaderElection == nil {
				mSpec.LeaderElection = &configv1alpha1.LeaderElectionConfiguration{}
			}

			mSpec.LeaderElection.ResourceLock = v
			processedArgs[k] = true
		...
		case k == "--diagnostics-address":
			mSpec.Metrics.DiagnosticsAddress = v
			processedArgs[k] = true

		case k == "--insecure-diagnostics":
			if val, err := strconv.ParseBool(v); err == nil {
				mSpec.Metrics.InsecureDiagnostics = val
				processedArgs[k] = true
			}
		...
		case k == "--feature-gates":
			if mSpec.FeatureGates == nil {
				mSpec.FeatureGates = make(map[string]bool)
			}

			features := strings.Split(v, ",")
			for _, feature := range features {
				// Use SplitN to split only at the first "=" to handle feature gate names that contain "="
		...
		default:
			// All unknown args are added to AdditionalArgs
			if mSpec.AdditionalArgs == nil {
				mSpec.AdditionalArgs = make(map[string]string)
			}

			mSpec.AdditionalArgs[k] = v
			processedArgs[k] = true
		}
	}
	...
	if mSpec.Controller == nil &&
		...
		mSpec.Health.LivenessEndpointName == "" &&
		mSpec.Health.ReadinessEndpointName == "" &&
		mSpec.GracefulShutdownTimeout == nil &&
