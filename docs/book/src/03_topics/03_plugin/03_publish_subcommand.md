...
This example demonstrates how to publish three different providers (`control-plane kubeadm`, `bootstrap kubeadm`, and `infrastructure docker`) along with their versioned metadata and components files into a **single OCI image**. Each provider has two versions (`v1.13.0` and `v1.14.0`), and the corresponding metadata and components files follow versioned naming conventions.

The following layout for the directory can be used:

```bash
manifests/
├── control-plane-kubeadm-v1.13.0-metadata.yaml
├── control-plane-kubeadm-v1.13.0-components.yaml
├── bootstrap-kubeadm-v1.13.0-metadata.yaml
├── bootstrap-kubeadm-v1.13.0-components.yaml
├── infrastructure-docker-v1.13.0-metadata.yaml
├── infrastructure-docker-v1.13.0-components.yaml
├── control-plane-kubeadm-v1.14.0-metadata.yaml
├── control-plane-kubeadm-v1.14.0-components.yaml
├── bootstrap-kubeadm-v1.14.0-metadata.yaml
├── bootstrap-kubeadm-v1.14.0-components.yaml
└── infrastructure-docker-v1.14.0-metadata.yaml
└── infrastructure-docker-v1.14.0-components.yaml
```
...
This will publish both versions (`v1.13.0` and `v1.14.0`) of each provider into single OCI image, and each version will have its corresponding metadata and component files.
